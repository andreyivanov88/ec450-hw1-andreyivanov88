...
interrupt void WDT_interval_handler(){
	--blink_counter;
  if ( blink_counter==0 && c < 5){          // decrement the counter and act only if it has reached 0
  ...
  if (blink_counter==0 && c == 5){          // decrement the counter and act only if it has reached 0
  	  P1OUT ^= 1;                   // toggle LED on P1.0
  	  blink_counter=blink_interval*3; // reset the down counter
  	  c++;
    }
  if (blink_counter==0 && c > 5 && c < 11){          // decrement the counter and act only if it has reached 0
    	  P1OUT ^= 1;                   // toggle LED on P1.0
    	  blink_counter=blink_interval*3; // reset the down counter
    	  c++;
      }
  if (blink_counter==0 && c == 11){          // decrement the counter and act only if it has reached 0
      	  P1OUT ^= 1;                   // toggle LED on P1.0
      	  blink_counter=blink_interval*3; // reset the down counter
      	  c++;
        }
  if (blink_counter==0 && c > 11 && c < 17){          // decrement the counter and act only if it has reached 0
        	  P1OUT ^= 1;                   // toggle LED on P1.0
        	  blink_counter=blink_interval; // reset the down counter
        	  c++;
          }
  if (blink_counter==0 && c == 17){          // decrement the counter and act only if it has reached 0
        	  P1OUT ^= 1;                   // toggle LED on P1.0
        	  blink_counter=blink_interval*3; // reset the down counter
        	  c++;
          }
  if (blink_counter==0 && c == 18){          // decrement the counter and act only if it has reached 0
          	  //P1OUT ^= 1;                   // toggle LED on P1.0
          	  blink_counter=blink_interval*7; // reset the down counter
          	  c = 0;;
            }
}
